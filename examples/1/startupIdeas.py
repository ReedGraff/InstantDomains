...
class DomainIdea(SchemicModel):
    """A single domain name idea with its ELO score."""
    idea: str = Field(..., description="A creative and brandable domain name idea. This should be a single word without the TLD (e.g., 'zenith', 'brightwork', 'catalyst').")
    elo: int = Field(..., description="A score from 0 to 999 representing the quality of the name. Higher is better.", ge=0, le=999)

class DomainNameIdeas(SchemicModel):
    """A model to hold a list of generated domain name ideas from the AI."""
    ideas: List[DomainIdea] = Field(
        ..., 
        description="A list of creative and brandable domain name ideas, each with an ELO score."
    )

# --- Functions for AI Interaction ---

def get_domain_ideas(prompt: str, num_ideas: int = 15) -> List[DomainIdea]:
    """
    ...
    Returns:
        List[DomainIdea]: A list of domain name ideas, each with an ELO score.
    """
    ...
    try:
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": """You are an expert in branding and naming startups. Generate a diverse list of short, memorable, and unique names based on the user's idea. For each name, provide an 'elo' score between 0 and 999, where a higher score indicates a better, more brandable name.

To generate ideas, use the following techniques:
0.  Exhaust all combinations of key terms in the description. Try using the terms most specific to the startup's industry. So for a company working in the logistics space, you might get 'LogisticsPro', 'CargoFlow', 'ShipSmart', 'FreightWise', etc. Not something like 'Fixify' or 'SuperFixer' or 'ServiceX' or 'FixFlow'. Also adding the most common terms in the industry so for something in the car dealership space you might want to include terms like 'auto' or 'autopart' or 'automobile' or 'dealer' or 'dealership' or 'car'.
...
"""},
                {"role": "user", "content": f"My startup idea is: {prompt}. Please give me a list of {num_ideas} potential names, each with an elo score."}
            ],
...
async def main(startup_idea: str, allowed_tlds: Set[str], only_show_available: bool, get_suggestions: bool, num_ideas: int):
    ...
    # Create a lookup for elo scores
    elo_map = {idea.idea: idea.elo for idea in ideas_from_gpt}
    ...
    try:
        ...
        tasks = [client.domain_search.search(idea.idea, allowed_tlds, get_suggestions=get_suggestions) for idea in ideas_from_gpt]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        # Process results
        for res in results:
            if isinstance(res, Exception):
                logger.error(f"Error during domain search: {res}", exc_info=True)
                continue
                ...
                if any(domain_info.domain.endswith(tld) for tld in allowed_tlds):
                    ...
                    if domain_info.domain not in all_results_data:
                         all_results_data[domain_info.domain] = {
                            ...
                            "available": domain_info.is_available,
                            "elo": 0 # Suggestions don't have an elo
                        }
    ...
    # Sort by elo score (descending), then alphabetically by domain name
    final_data.sort(key=lambda x: (-x["Elo"], x["Domain"]))
    ...
    try:
        with open(output_filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ["Domain", "Elo", "Source", "Status"]
